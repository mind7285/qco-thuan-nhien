...
class M_Tb_Auth_Mod extends M_Db_Guid_Seq_Itm {
  // 🇻🇳 Tên module hiển thị
  // 🇺🇸 Display module name
  final String c_mod_name;

  // 🇻🇳 Mã module để check logic
  // 🇺🇸 Module code for logic checking
  final String c_mod_code;

  // 🇻🇳 Danh sách quyền thuộc module
  // 🇺🇸 List of permissions belonging to module
  final List<M_Tb_Auth_Perm>? perms;

  const M_Tb_Auth_Mod({
    required super.q_id,
    super.q_seq = 1,
    required this.c_mod_name,
    required this.c_mod_code,
    this.perms,
    super.q_status = 0,
    super.q_version = 0,
  ...
  factory M_Tb_Auth_Mod.fromJson(Map<String, dynamic> json) {
    return M_Tb_Auth_Mod(
      q_id: json['q_id'] as String,
      q_seq: json['q_seq'] as int? ?? 1,
      c_mod_name: json['c_mod_name'] as String,
      c_mod_code: json['c_mod_code'] as String,
      perms: json['perms'] != null
          ? (json['perms'] as List)
              .map((e) => M_Tb_Auth_Perm.fromJson(e as Map<String, dynamic>))
              .toList()
          : null,
  ...
  Map<String, dynamic> toJson() {
    return {
      'c_mod_name': c_mod_name,
      'c_mod_code': c_mod_code,
      'perms': perms?.map((e) => e.toJson()).toList(),
      ...super.toJson(),
    };
  }
  ...
  M_Tb_Auth_Mod copyWith({
    ...
    return M_Tb_Auth_Mod(
      ...
      c_mod_name: c_mod_name ?? this.c_mod_name,
      c_mod_code: c_mod_code ?? this.c_mod_code,
